...
const verified = verifyPreparedRelease({ pkg, record: JSON.parse(readFileSync(`${path}.release.json`)), archive: readFileSync(path), checksum: readFileSync(`${path}.sha256`, 'utf8').split(' ')[0], head, dirty: execFileSync('git', ['status', '--porcelain'], options).trim(), ci });
...
if (args[0] !== '--publish') console.log(existing ? 'The exact archive is already published.' : 'Ready. Run npm run release:publish -- --publish to publish this exact archive.');
  ...
  if (!existing) {
    try { npm(['publish', path, '--access', 'public', '--ignore-scripts', ...registry], { cwd: root, stdio: 'inherit' }); }
