...
- The GitHub CI matrix runs the same suite on Node.js 22/24 and macOS/Linux. Published-source installation can be reproduced with `npm run test:hosts -- --github` after pushing the release.
...
- GitHub: authenticated repository read access and Git fetch succeeded against `Zachshotamartin/just-vibe`. Release push, remote installation and CI are checked separately against the published revision.
