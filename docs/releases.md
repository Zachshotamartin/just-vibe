...
6. Add `evals/releases/VERSION-publication.json` with the registry integrity, shasum, publish time and the validation actually performed. Validation fails when a `vVERSION` tag has no record, and the website derives its current version from these records.
...
After registry integrity and exact-version execution are verified, publish the corresponding website build so its generated `/release.json` describes the released package. The Vercel project is connected to this repository, so a merge to `main` also deploys production. The site states the latest version that has a publication record and marks a newer source version as unreleased, but merge release-note and install-command changes only after npm publication. Portfolio consumers validate its schema, fixed URLs, install command and monotonic version against a checked-in fallback, and confirm the version exists in npm before accepting an update.
...
Record the reason, maintainer authorization, source commit, artifact hashes, local checks and outstanding platform coverage. Tag the actual published source revision. Never overwrite a published version or weaken the normal CI check for future releases. The [0.8.0 publication record](https://github.com/Zachshotamartin/just-vibe/blob/main/evals/releases/0.8.0-publication.json), [0.8.1 publication record](https://github.com/Zachshotamartin/just-vibe/blob/main/evals/releases/0.8.1-publication.json) [0.9.0 publication record](https://github.com/Zachshotamartin/just-vibe/blob/main/evals/releases/0.9.0-publication.json) [0.10.0 publication record](https://github.com/Zachshotamartin/just-vibe/blob/main/evals/releases/0.10.0-publication.json) and [0.11.0 publication record](https://github.com/Zachshotamartin/just-vibe/blob/main/evals/releases/0.11.0-publication.json) document separately authorized release-specific exceptions.
