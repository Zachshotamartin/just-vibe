...
The first fresh Linux CI browser run on `d65305e` correctly rejected the exported hidden-text fixture, but its test harness required one exact diagnostic. Under CI timing, the deadline instead expired during a subsequent visibility check. The harness now requires exit 1, the `executeQaStep` failure frame and either legitimate text/visibility deadline diagnostic. It also reruns the same exported test after restoring visible text and requires both assertions to pass. No production assertion, timeout or acceptance requirement was relaxed. The release proceeds only after the updated source passes fresh CI and exact-archive preparation.

The next Windows Node 22 run reported `EBUSY` in fixture cleanup after the owned-service assertions passed. The supervisor writes its terminal state immediately before exiting, while Windows holds its working directory until process exit. The service test now waits for and asserts that the recorded supervisor actually exits before deleting the fixture. It does not kill a stale PID, ignore cleanup errors or weaken the service-state/output assertions.
