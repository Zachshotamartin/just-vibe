...
7. **Trusted publication.** Verify npm trusted-publisher configuration and the existing tag-bound OIDC workflow, run complete release preparation from a clean commit, and verify the exact published artifact if a release is performed. Never label a dry run a successful OIDC publication. Account steps requiring user interaction remain explicit.
