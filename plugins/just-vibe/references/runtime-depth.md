...
Zed receives compact skill descriptions under `.agents/skills`; host worktree trust and catalog limits apply. OpenClaw reads the same `.agents/skills` folder, so one installation serves both hosts: installing the second target reports which adapter already provides the skills, and update or uninstall goes through that adapter. Uninstall also removes the emptied `just-vibe-*` folders. Hermes receives `skills/just-vibe-*` under the explicitly supplied **actual Hermes home**, not an arbitrary project folder. Hermes is installed separately from the project-oriented wizard. These two targets have file lifecycle support, not new hook adapters. Neither adapter changes existing user skills or provider credentials.
