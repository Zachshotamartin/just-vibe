...
- Bounded local execution (any mode): the project's own dev, build and test commands and short local profiling may run as bounded reproduction when they bind only to localhost, contact no shared or remote service, install nothing, and write only to ignored cache or build output or an owned temporary directory. Redirect output that would replace saved artifacts to an owned temporary directory (for Vite, `vite build --outDir <tmp> --emptyOutDir`). Probe edits happen only in an owned copy or worktree. Tracked-file writes, installs, shared databases, load generation and paid resources need apply mode or explicit action authorization. In a tracked run, record such execution as a read stage when it writes nothing inside the project scope.
...
Set explicit time/data/compute limits for load, tuning, training, broad scans, backfills, or paid inference. Default orchestration: eight stages and up to three attempts per stage (initial plus two corrective retries), with no wall-clock limit unless the user sets one. A retry requires new evidence. Do not reset counters to evade a limit; an explicit new budget can start a continuation run that links prior evidence through `context.continuationOf` (see [runtime](runtime.md#session-operations)). Do not promise future monitoring without a real authorized scheduler or job handle.
