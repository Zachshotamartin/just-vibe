...
`lab cleanup NAME --stdin` takes `revision` and `expected`, mapping each existing variant ID to its complete `snapshot` object from a freshly reviewed show result (including content, index and Git identity). Older content-hash strings are rejected; retrieve fresh snapshots before retrying. This request intentionally removes those owned workspaces, including their known variant changes and disposable dependency/build output. Preserve any variant worth keeping first. Staged changes and new ignored/private content outside cleanup coverage block removal. Preserve staged work explicitly before cleanup; a fresh snapshot alone does not permit discarding it. All existing workspaces are checked before any removal, then checked again immediately before deleting each one. Changed snapshots, partial coverage and invalid workspace identity also block cleanup. Cleanup never removes the original worktree.
