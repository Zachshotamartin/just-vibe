...
3. Create a QA plan with exact target, request-linked criteria, viewport and ordered actions/assertions. For an upload, assert processing completion and advancing media playback, not merely the existence of an upload element. For invalid files, assert visible understandable error content. For mobile, check horizontal overflow, specified control clipping and overlap; retain a screenshot for human visual review.
